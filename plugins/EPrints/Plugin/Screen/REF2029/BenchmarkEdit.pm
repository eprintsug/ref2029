package EPrints::Plugin::Screen::REF2029::BenchmarkEdit;
...
sub action_save
{
    ...
    # TODO: Display warnings??? (It's only a benchmark....)
}
...
sub render_title
{
    ...
    return $self->html_phrase( 'title' ) unless( defined $benchmark );

    return $self->html_phrase( 'benchmark_title', title => $benchmark->render_value( 'title' ) );
}
...
sub properties_from
{
    ...
    my $dataset = $self->{processor}->{dataset} = $self->{session}->dataset( "ref2029_benchmark" );

    my $benchmarkid = $self->{session}->param( "benchmarkid" );
    $self->{processor}->{benchmarkid} = $benchmarkid;
    $self->{processor}->{benchmark} = $dataset->dataobj( $benchmarkid );

    if( !defined $self->{processor}->{benchmark} )
    {
...
sub redirect_to_me_url
{
    ...
    return $self->SUPER::redirect_to_me_url."&benchmarkid=".$self->{processor}->{benchmarkid};
}
...
sub hidden_bits
{
    ...
    return(
        ...
        benchmarkid => $self->{processor}->{benchmarkid},
    );
}
